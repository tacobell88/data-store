...
class NRIC {
  ...
  static Generate(firstchar = null) {
    ...
    const getRandomFirstChar = () =>
      "ST"
        .split("")
        .sort(() => 0.5 - Math.random())
        .pop();
    firstchar = /^[STFGM]$/i.test(firstchar)
  ...
  static Validate(value) {
    return Array.isArray(value)
      ? value.every((item) =>
          item instanceof NRIC ? item.isValid : new NRIC(item).isValid
        )
      : value instanceof NRIC
      ? value.isValid
      : new NRIC(value).isValid;
  }
  ...
  static #calculateChecksum(firstchar, digits) {
    ...
    const offset =
      firstchar === "T" || firstchar === "G" ? 4 : firstchar === "M" ? 3 : 0;
